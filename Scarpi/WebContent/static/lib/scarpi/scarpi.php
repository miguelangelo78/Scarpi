...
class Scarpi{
	...
	public function scrape($url, $targets, $render){
		$this->render = strtolower(trim($render)) == 'true';
	...
	private function parse(&$html, &$targets){
		...
		foreach($targets as $target){
			...
			// Scrape:
			$col_name = trim($options[0]);
			$res_container[$col_name] = array();
			...
			foreach($elements as $el){
				...
				$what_to_push = null;
				switch($target_mode){
					case "html": $what_to_push = ($this->render) ? stripslashes($el->outertext) : htmlspecialchars($el->outertext); break;
				...
				if(!empty($what_to_push)){
					array_push($res_container[$col_name], $what_to_push);
					$added_count++;
				}
